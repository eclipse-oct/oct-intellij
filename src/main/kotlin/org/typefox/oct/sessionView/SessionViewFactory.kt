...
class NoSessionView(val project: Project?): JPanel() {
    ...
    private fun executeAction(actionId: String) {
        ...
        if (action != null) {
            val event = AnActionEvent.createEvent(action, if (project != null) SimpleDataContext.getProjectContext(project) else SimpleDataContext.builder().build(), null, ActionPlaces.UNKNOWN, ActionUiKind.NONE, null)
